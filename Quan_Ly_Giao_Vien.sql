...
USE master
GO
IF exists (SELECT * FROM sysdatabases WHERE name = 'QLGV')
DROP DATABASE QLGV
GO
...
-- TẠO KHÓA NGOẠI
ALTER TABLE dbo.HOCVIEN ADD FOREIGN KEY (MALOP) REFERENCES dbo.LOP(MALOP) 

ALTER TABLE dbo.LOP ADD FOREIGN KEY (MAGVCN) REFERENCES dbo.GIAOVIEN(MAGV) 
ALTER TABLE dbo.LOP ADD FOREIGN KEY (TRGLOP) REFERENCES dbo.HOCVIEN(MAHV) 

ALTER TABLE dbo.KETQUATHI ADD FOREIGN KEY (MAMH) REFERENCES dbo.MONHOC(MAMH) 
ALTER TABLE dbo.KETQUATHI ADD FOREIGN KEY (MAHV) REFERENCES dbo.HOCVIEN(MAHV) 
 
ALTER TABLE dbo.MONHOC ADD FOREIGN KEY (MAKHOA) REFERENCES dbo.KHOA(MAKHOA) 

ALTER TABLE dbo.DIEUKIEN ADD FOREIGN KEY(MAMH_TRUOC) REFERENCES dbo.MONHOC(MAMH) 
ALTER TABLE dbo.DIEUKIEN ADD FOREIGN KEY (MAMH) REFERENCES dbo.MONHOC(MAMH) 
...
ALTER TABLE dbo.GIANGDAY ADD FOREIGN KEY (MALOP) REFERENCES dbo.LOP(MALOP) 
...
ALTER TABLE dbo.HOCVIEN ADD GHICHU VARCHAR(20)
ALTER TABLE dbo.HOCVIEN ADD DIEMTB NUMERIC(3,1)
ALTER TABLE dbo.HOCVIEN ADD XEPLOAI VARCHAR(10) 
-- CÁCH KHÁC: ALTER TABLE dbo.HOCVIEN ADD GHICHU VARCHAR(50), DIEMTB NUMERIC(4,2), XEPLOAI VARCHAR(10)

--2.Mã học viên là một chuỗi 5 ký tự, 3 ký tự đầu là mã lớp, 2 ký tự cuối cùng là số thứ tự học viên trong lớp. VD: “K1101”
ALTER TABLE HOCVIEN ADD CONSTRAINT ck_MaHV CHECK (LEFT(MAHV,3)= MALOP AND (RIGHT(MAHV,2) >= 1 AND RIGHT(MAHV,2) <= 30))

--3. Thuộc tính GIOITINH chỉ có giá trị là “Nam” hoặc “Nu”.
ALTER TABLE HOCVIEN ADD CONSTRAINT ck_gt CHECK (GIOITINH = 'Nam' OR GIOITINH = 'Nu')
ALTER TABLE GIAOVIEN ADD CONSTRAINT ck_gt1 CHECK (GIOITINH = 'Nu' OR GIOITINH = 'Nam')
-- CÁCH KHÁC: ALTER TABLE HOCVIEN ADD CONSTRAINT CHECK_gt CHECK(GIOITINH IN('Nam', 'Nu'))

--4. Điểm số của một lần thi có giá trị từ 0 đến 10 và cần lưu đến 2 số lẽ (VD: 6.22)
ALTER TABLE KETQUATHI ADD CONSTRAINT ck_DIEM CHECK (DIEM BETWEEN 0 AND 10 AND RIGHT(CAST(DIEM AS VARCHAR), 3) LIKE '._')

--5. Kết quả thi là “Dat” nếu điểm từ 5 đến 10 và “Khong dat” nếu điểm nhỏ hơn 5.
ALTER TABLE KETQUATHI ADD CONSTRAINT ck_KQT CHECK ((KQUA = 'Dat' AND DIEM >=5 AND DIEM <=10) OR (KQUA = 'Khong dat' AND DIEM < 5))
-- CÁCH KHÁC: ALTER TABLE KETQUATHI ADD CONSTRAINT ck_KQT CHECK ((KQUA = 'Dat' AND DIEM BETWEEN 5 AND 10) OR (KQUA = 'Khong dat' AND DIEM < 5))

--6. Học viên thi một môn tối đa 3 lần.
ALTER TABLE KETQUATHI ADD CONSTRAINT ck_LT CHECK (LANTHI <= 3)

--7. Học kỳ chỉ có giá trị từ 1 đến 3.
ALTER TABLE GIANGDAY ADD CONSTRAINT ck_HK CHECK (1 <= HOCKY AND HOCKY <= 3)
-- CÁCH KHÁC: ALTER TABLE GIANGDAY ADD CONSTRAINT ck_HK CHECK (HOCKY BETWEEN 1 AND 3)

--8. Học vị của giáo viên chỉ có thể là “CN”, “KS”, “Ths”, ”TS”, ”PTS”
ALTER TABLE GIAOVIEN ADD CONSTRAINT ck_HOCVI CHECK (HOCVI IN ('CN', 'KS', 'Ths', 'TS', 'PTS'))

--9. Lớp trưởng của một lớp phải là học viên của lớp đó
ALTER TABLE LOP ADD CONSTRAINT ck_loptruong CHECK (LEFT(TRGLOP,3) = MALOP)
--10.Trưởng khoa phải là giáo viên thuộc khoa và có học vị “TS” hoặc “PTS”.--11. Học viên ít nhất là 18 tuổi.ALTER TABLE HOCVIEN ADD CONSTRAINT ck_TUOI CHECK (YEAR(GETDATE()) - YEAR(NGSINH) >= 18)--12.Giảng dạy một môn học ngày bắt đầu (TUNGAY) phải nhỏ hơn ngày kết thúc (DENNGAY)ALTER TABLE GIANGDAY ADD CONSTRAINT ck_THGIAN CHECK (TUNGAY < DENNGAY)--13.Giáo viên khi vào làm ít nhất là 22 tuổi.ALTER TABLE GIAOVIEN ADD CONSTRAINT ck_TUOIGV CHECK (YEAR(NGVL) - YEAR(NGSINH) >= 22)--14.Tất cả các môn học đều có số tín chỉ lý thuyết và tín chỉ thực hành chênh lệch nhau không quá 3.ALTER TABLE MONHOC ADD CONSTRAINT ck_CHENHLECH CHECK (ABS(TCLT - TCTH) <= 3)--15.Học viên chỉ được thi một môn học nào đó khi lớp của học viên đã học xong môn học này--16.Mỗi học kỳ của một năm học, một lớp chỉ được học tối đa 3 môn.--17.Sỉ số của một lớp bằng với số lượng học viên thuộc lớp đó.--18.Trong quan hệ DIEUKIEN giá trị của thuộc tính MAMH và MAMH_TRUOC trong cùng một bộ không được giống nhau (“A”,”A”) và cũng không tồn tại hai bộ (“A”,”B”) và
...
INSERT INTO KETQUATHI VALUES ('K1305','CTRR',1,'13/5/2006',10,'Dat')SELECT * FROM KETQUATHI--II. Ngôn ngữ thao tác dữ liệu --1. Tăng hệ số lương thêm 0.2 cho những giáo viên là trưởng khoa. (HESO = HESO + 0.2) AND 
ALTER TABLE GIAOVIEN, KHOA ADD CONSTRAINT ck_GV_K CHECK (HESO = HESO + 0.2) AND (KHOA.TRGKHOA = GIAOVIEN.MAGV)
...
--o Nếu DIEMTB < 5 thì XEPLOAI = ”Y”--III.NGÔN NGỮ TRUY VẤN DỮ LIỆU (1- 14)--1. In ra danh sách (mã học viên, họ tên, ngày sinh, mã lớp) lớp trưởng của các lớp.
SELECT L.MALOP, MAHV, HO, TEN, NGSINH
...
FROM KETQUATHI KT, MONHOC MH, HOCVIEN HV
WHERE LANTHI = 1 AND KQUA = 'Dat' AND KT.MAMH = MH.MAMH AND HV.MAHV = KT.MAHV
-- cach 2
SELECT HV.MAHV, HV.HO, HV.TEN, MH.TENMH
FROM KETQUATHI KT join MONHOC MH on ... join HOCVIEN HV on ...
WHERE LANTHI = 1 AND KQUA = 'Dat' AND KT.MAMH = MH.MAMH AND HV.MAHV = KT.MAHV

--4. In ra danh sách học viên (mã học viên, họ tên) của lớp “K11” thi môn CTRR không đạt (ở lần thi 1).
--5. * Danh sách học viên (mã học viên, họ tên) của lớp “K” thi môn CTRR không đạt (ở tất cả các lần thi).
--6. Tìm tên những môn học mà giáo viên có tên “Tran Tam Thanh” dạy trong học kỳ 1 năm 2006.SELECT COUNT(MAHV) SL FROM KETQUATHI WHERE MAMH = 'CSDL' AND DIEM >= 5SELECT TOP 1 WITH TIES DIEM, MAHV FROM KETQUATHI ORDER BY DIEM DESCSELECT MAHV, DIEMFROM KETQUATHIWHERE DIEM >= 7 (SELECT DIEM FROM KETQUATHI)